...
We maintain a fork of `tomasz-tomczyk/crit` with custom changes that need to survive each upstream merge:
...
3. **Code-font parse timeout** (added v0.19.1) — `go-text/typesetting@v0.3.4` hangs (not panics) parsing some real-world fonts, e.g. macOS's bundled `SFIndia.ttc`. Our fix wraps each font parse in `internal/server/code_fonts.go`'s `safelyInspectCodeFont` with a 2s timeout via a goroutine + `select`/`time.After`, since the pre-existing `recover()` only guards against panics, not hangs. Upstream does NOT have this fix — re-check if it's still needed on future merges (upstream may bump the typesetting dep or add its own guard).
4. **No default share target** (formalized v0.20.1, policy predates it) — deliberate: this fork must never let a share/pull/unpublish/auth command silently talk to a server the user hasn't explicitly configured. Upstream's `internal/config/share_targets.go` `ResolveShareTargets()` defaults to the public `https://crit.md` target when nothing is configured (its `default:` case). Our fix in `internal/config/config.go`'s `LoadConfig()` step 4 forces `shareURLPresent = true` (and leaves `ShareURL`/`ShareTargets` at their zero values) whenever `share_targets` wasn't itself explicitly configured, so `ResolveShareTargets` takes its `case cfg.shareURLPresent: ShareURL == "" → return []` branch instead — zero targets, sharing commands fail closed with "sharing is disabled; configure a share target or pass --share-url" rather than falling back to crit.md. `defaultConfig()` also keeps `ShareURL: ""` / no `ShareTargets` in the `crit config --generate` template (upstream defaults that template to a `crit.md` entry). A consent prompt (`ShareTarget.NeedsShareConsent`) is NOT considered sufficient on its own — it can be misclicked or piped through non-interactively, so the fork removes the default target entirely rather than relying on the prompt as the safety boundary.
   - Before this was formalized, the fork got the equivalent effect for free from architectural quirks of the pre-v0.20.1 code (a CLI-only `ResolveShareURL(..., config.DefaultShareURL)` fallback vs. the browser-facing path reading raw `cfg.ShareURL` with no such fallback) — that accidental split is gone now that everything routes through `ResolveShareTargets`, so this is now a single, explicit, intentional policy enforced in one place.
   - Tests that assumed upstream's implicit default (added upstream in the v0.20.1 range) were updated to explicitly configure a `share_url`/`share_targets` in their test fixtures instead of relying on the default: `cmd/crit/main_test.go` `TestRunShare_ConsentDenied`, `internal/share/share_test.go` `TestRunShare_BlockedByProxyAuth`, `internal/auth/auth_test.go` `TestRunAuthWhoami_NotLoggedIn` / `TestRunAuthLogout_NotLoggedIn` / `TestRunAuthLogout_EnvToken`. Do NOT "fix" these tests by re-adding an implicit default on a future merge.
...
| v0.19.1 | 2026-09-03 | 1 file, a genuine logic conflict (not import-only): `internal/live/proxy.go`'s `Rewrite` func — the fork had independently applied the same `Director`→`Rewrite` migration at v0.19.0 (see that row's first gotcha), and upstream did the same migration independently in this range with more functionality (`pr.SetXForwarded()`, `X-Forwarded-Port` header, a proper `rewriteReferer()` helper instead of a bare Origin swap). Took upstream's version wholesale — a strict superset, no fork-specific logic to preserve there. Custom changes verified intact: IsSessionFile, handleFiles IsAbs block, ShareURL default, no homebrew tap step, settings-panel.spec.ts Account/Share assertions (all auto-merged cleanly). | Merged through the v0.19.1 tag (upstream had 9 more unreleased commits past it, deferred to next merge). One new upstream Go file (`internal/focus/focus_cli_test.go`) needed the `tomasz-tomczyk`→`JoshEllinger` import fix. go.mod/go.sum unchanged, so `flake.nix`'s `vendorHash` needed no update; `version` auto-merged cleanly to `0.19.1`. `go generate ./...` produced no drift. **Real bug found (not a merge regression, not fork-introduced):** upstream's new code-font Settings-picker feature (`internal/server/code_fonts.go`, using `go-text/typesetting@v0.3.4`) hung indefinitely — not panicked, actually hung — parsing `/System/Library/Fonts/SFIndia.ttc`, a font Apple ships standard on modern macOS. Confirmed via an isolated probe script iterating all 835 local system-font footprints with a per-font timeout: only this one font hung. Existing code only had `recover()` panic guards, no timeout, so this would have hung any real request to the Settings font picker on most Macs. Fixed in a follow-up commit (`internal/server/code_fonts.go`'s `safelyInspectCodeFont`) by running each font parse in a goroutine with a 2s timeout via `select`/`time.After` — accepted as a fork-local addition since it's a real-world hang bug upstream's own test suite likely doesn't catch on CI runners that lack this font. This is a new fork customization to track on future merges (in addition to the four listed at the top of this doc) if upstream's own fix ever lands. Full `go build`, `go test ./...` (all packages, including the previously-hanging `TestDiscoverCodeFontFamiliesScansTheCurrentSystem` — now completes in ~18s), `go vet ./...`, and `gofmt -l .` clean. |
| v0.20.1 | 2026-09-14 | 11 files. 10 were pure import-block conflicts (upstream's `internal/forge`/`internal/focus`/`internal/daemon` imports added or removed across cmd/crit/wire_test.go, internal/comment/comment_scope_test.go, internal/github/{export,github_test,pull_cli,push_cli,push_cli_flags_test,push_partial_failure_test}.go, internal/server/helpers.go, internal/share/share_integration_test.go) — resolved by taking upstream's import list (checking each HEAD-only import for actual usage first; several, e.g. `daemon`/`vcs`/`auth` in test files, were dead and upstream had already dropped them) and translating `tomasz-tomczyk`→`JoshEllinger`. The 11th, `internal/config/config.go`, was a genuine logic conflict from upstream's new multi-target sharing feature (`feat: support multiple share targets across Crit deployments`, #904): upstream's `defaultConfig()` and `LoadConfig()` step 4 now default `ShareURL`/`ShareTargets` to the public `https://crit.md` target when nothing is configured. Resolved by keeping the fork's no-default-share-target policy (see customization #4 at the top of this doc) — `defaultConfig()` keeps `ShareURL: ""`/no `ShareTargets`; `LoadConfig()` forces `shareURLPresent = true` when `share_targets` wasn't explicitly configured so `ResolveShareTargets()` resolves to zero targets rather than falling back to crit.md. Custom changes verified intact: IsSessionFile, handleFiles IsAbs block, code-font timeout, no homebrew tap step, settings-panel.spec.ts assertions (auto-merged cleanly, unaffected — the "no targets" case already matched its expectations). | Merged through the v0.20.1 tag (40 upstream commits since v0.19.1; 7 more unreleased past it, deferred to next merge). The no-default-share-target resolution above broke 5 of upstream's own new/existing tests that assumed the implicit crit.md default: `cmd/crit/main_test.go` `TestRunShare_ConsentDenied`, `internal/share/share_test.go` `TestRunShare_BlockedByProxyAuth`, `internal/auth/auth_test.go` `TestRunAuthWhoami_NotLoggedIn`/`TestRunAuthLogout_NotLoggedIn`/`TestRunAuthLogout_EnvToken` — all fixed by adding an explicit `share_url` to each test's fixture config rather than relying on the default, per user direction (a consent prompt is not a sufficient block against sharing to an uncontrolled server; the default must not exist at all). `test/e2e/tests/share-transport.sharetransport.spec.ts` and its fixture were substantially rewritten upstream for multi-target support but already configure `share_targets` explicitly — unaffected by the policy, left as-is (not re-verified live; E2E deferred to CI per the v0.18.2 precedent). go.mod gained one new dependency version bump (`github.com/go-text/typesetting` to a newer pre-release pseudo-version, fixing a CFF2 INDEX allocation bug, #897) — left `flake.nix`'s `vendorHash` untouched for CI's `nix build` to validate, no local Nix available. `flake.nix` `version` auto-merged cleanly to `0.20.1`. `go generate ./...` produced no drift. Full `go build`, `go test ./...` (all packages), `go vet ./...`, and `gofmt -l .` clean. |
...
- **internal/config/config.go** — `defaultConfig()` has `ShareURL: ""` and no `ShareTargets` (fork intentionally suppresses the crit.md default). `LoadConfig()` step 4 forces `shareURLPresent = true` when `share_targets` wasn't explicitly configured, so `ResolveShareTargets()` never falls back to `https://crit.md` (see customization #4 at the top of this doc). Upstream may add new fields (e.g. `OpenCmd`, `ShareTargets`). Take new upstream fields, keep the no-default-target policy — do NOT adopt upstream's "default to crit.md when unconfigured" logic.
...
- **internal/server/code_fonts.go** — Our `safelyInspectCodeFont` timeout wrapper (added v0.19.1). If upstream rewrites this file, re-check whether the SFIndia.ttc-class hang is still reproducible (run `go test ./internal/server/... -run TestDiscoverCodeFontFamilies -timeout 60s`) before deciding whether to keep, drop, or re-port the fix.
...
# Code-font parse timeout
grep -n 'codeFontInspectTimeout' internal/server/code_fonts.go

# No homebrew tap push
grep -n 'homebrew\|HOMEBREW_TAP' .github/workflows/release.yml  # expect NO output

# No default share target (run `crit config` with an empty temp HOME; expect
# share_url/share_targets absent, not defaulted to https://crit.md)
grep -n 'shareURLPresent = globalPresence.ShareURL' internal/config/config.go
```
