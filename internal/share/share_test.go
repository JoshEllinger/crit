...
func TestRunShare_BlockedByProxyAuth(t *testing.T) {
	...
	// proxy_auth only makes sense against an explicitly configured self-hosted
	// server, so the config must set share_url for a target to resolve at all.
	if err := os.WriteFile(filepath.Join(homeDir, ".crit.config.json"), []byte(`{"proxy_auth":true,"share_url":"https://crit.internal.example.com"}`), 0644); err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
