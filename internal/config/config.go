...
func defaultConfig() generatedConfig {
	return generatedConfig{
		// Fork customization: ShareURL/ShareTargets default to empty (no default
		// sharing to crit.md); upstream defaults to the public crit.md target.
		Port:       0,
		Host:       "127.0.0.1",
		NoOpen:     false,
		OpenCmd:    "",
...
func LoadConfig(projectDir string) Config { //nolint:gocyclo // Config precedence is kept explicit for auditability.
	...
	// Fork customization: unlike upstream (which defaults to the public crit.md
	// target), this fork treats "nothing configured" as "explicitly no share
	// target" so ResolveShareTargets resolves to zero targets unless the user
	// opts in via share_url or share_targets.
	merged.shareTargetsPresent = globalPresence.ShareTargets
	merged.shareURLPresent = globalPresence.ShareURL || !globalPresence.ShareTargets
